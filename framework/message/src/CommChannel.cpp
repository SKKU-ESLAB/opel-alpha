...
bool CommChannel::start() {
  OPEL_DBG_VERB("Start CommChannel");
  ...
  if(!defaultOpenRes) {
    OPEL_DBG_ERR("CommChannel run: opening default port fail");
    return false;
  }
  if(!controlOpenRes) {
...
void CommChannel::stop() {
  OPEL_DBG_VERB("Stop CommChannel");
...
bool CommChannel::enableLargeDataMode() {
  ...
  if(!largeDataOpenRes) {
    OPEL_DBG_ERR("CommChannel enableLargeData: opening largedata port fail");
    return false;
  }
...
void CommChannel::onRoutedMessage(BaseMessage* message) {
  ...
  switch(state) {
    ...
    case CommChannelState::LISTENING_DEFAULT:
      OPEL_DBG_ERR("CommChannel has not initialized!");
      return;
      break;
    ...
    case CommChannelState::CONNECTED_LARGEDATA:
      ...
      break;
    default:
      OPEL_DBG_ERR("Unknown CommChannel state");
      return;
      break;
  }
...
void DataPortsListener::onReceivedRawMessage(std::string messageData,
  ...
  OPEL_DBG_VERB("Received raw message from CommChannel: %s / %s",
...
void CommChannel::onReceivedControlMessage(std::string messageData) {
  ...
  if(messageData.compare("on") == 0) {
    ...
    if(enableLargeDataRes) {
      ...
      if(getIpRes < 0) {
        OPEL_DBG_ERR("Cannot get IP address & WFD name");
        return;
      }
