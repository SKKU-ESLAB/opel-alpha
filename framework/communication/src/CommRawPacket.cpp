...
#define READ_SOCKET_S(socketFd, data) \
  ...
  data = ntohs(data);
#define READ_SOCKET_L(socketFd, data) \
  if(read(socketFd, &data, sizeof(long)) < 0) return NULL; \
  data = ntohl(data);
...
#define WRITE_BYTEARRAY_L(x) *x = htonl(*x); \
  memcpy(writePtr, x, sizeof(long)); \
...
char* CommRawPacketHeader::toByteArray() {
  ...
  WRITE_BYTEARRAY_L(&this->mCurrOffset);
  return byteArray;
}
...
char* CommPayloadMessageMetadata::toByteArray() {
  ...
  NEW_BYTEARRAY(byteArray, this->getBytesSize());
  WRITE_BYTEARRAY_L(&this->mMessageDataLength);
  WRITE_BYTEARRAY_L(&this->mIsFileAttached);
  return byteArray;
}
...
CommPayloadFileMetadata* CommPayloadFileMetadata::readFromSocket(int socketFd) {
  ...
  char fileNameLength;
  char* fileName;
  READ_SOCKET_L(socketFd, fileSize);
...
char* CommPayloadFileMetadata::toByteArray() {
  NEW_BYTEARRAY(byteArray, this->getBytesSize());
  WRITE_BYTEARRAY_L(&this->mFileSize);
