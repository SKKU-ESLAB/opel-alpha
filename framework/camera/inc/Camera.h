...
#if CONFIG_LOG_DBG
#define OPEL_LOG_DBG(fw, fmt, args...) do{ _opel_log(fmt, "[LOG] CAMERA FW", 94, ##args); }while(0)
#else
#define OPEL_LOG_DBG(fw, fmt, args...)
#endif

#if CONFIG_LOG_WARN
#define OPEL_LOG_WARN(fw, fmt, args...) do{ _opel_log(fmt, "[WARN] CAMERA FW", 91, ##args); }while(0)
#else
#define OPEL_LOG_WARN(fw, fmt, args...)
#endif

#if CONFIG_LOG_ERR
#define OPEL_LOG_ERR(fw, fmt, args...) do{ _opel_log(fmt, "[ERR] CAMERA FW", 101, ##args); }while(0)
#else
#define OPEL_LOG_ERR(fw, fmt, args...)
#endif
