...
CameraRequest::CameraRequest(RequestType type, GstElement *bin)
{
  ...
  if (this->mLastElement == NULL)
    OPEL_LOG_WARN(CAM, "This request bin has not set sink element");
...
{
  if (this->mTee == NULL)
    OPEL_LOG_ERR(CAM, "Tee is not set");
  else
...
void CameraRequest::linkPads()
{
  ...
  if (ret != GST_PAD_LINK_OK)
    OPEL_LOG_ERR(CAM, "Pad link failed (%d)", ret);
}
...
GstElement* CameraRequest::getElementByName(const char *name)
{
  ...
  OPEL_LOG_ERR(CAM, "Cannot find element by element name");
  return NULL;
}
