  ...
  while (!done) {
    switch (gst_iterator_next(iter, &item)) {
      ...
      case GST_ITERATOR_RESYNC:
        OPEL_LOG_WARN(CAM, "Datastructure changed while iterating");
        gst_iterator_resync(iter);
        break;
      case GST_ITERATOR_ERROR:
        OPEL_LOG_ERR(CAM, "Gstreamer iterator failed");
        done = TRUE;
        break;
...
bool CameraDevice::processRequest(CameraRequest *request)
{
  ...
  switch (type) {
    ...
    default:
      OPEL_LOG_ERR(CAM, "Invalid request type (%d)", type);
      break;
  }
...
bool CameraDevice::recordingStart(CameraRequest *request)
{
  ...
  if (this->mH264Tee == NULL) {
    OPEL_LOG_ERR(CAM, "Main bin have to include Tee name: %s", H264_TEE_NAME);
    return false;
  }
...
bool CameraDevice::preRecordingInit(CameraRequest *request)
{
  ...
  if (this->mDelayedTee == NULL)
    ...
    OPEL_LOG_ERR(CAM, "Pre-recording initialization already performed");
    return false;
  }
  
  this->mDelayedValve = request->getElementByName("valve");
  if (this->mDelayedValve == NULL) {
    OPEL_LOG_ERR(CAM, "Pre-recording requires a 'valve' element");
    return false;
  }
...
bool CameraDevice::preRecordingStart(CameraRequest *request)
{
  ...
  if (this->mDelayedTee == NULL) {
    OPEL_LOG_ERR(CAM, "Initialization of pre-recording should be performed first.");
    return false;
  }
...
{
  OPELRawRequest *raw_request = OPELRawRequest::getInstance(kOpenCVStart, NULL);
...
bool CameraDevice::openCVStop(CameraRequest *request)
{
  ...
  if (this->getOpenCVNumUsers() > 0) {
    OPEL_LOG_DBG(CAM, "OpenCV Bin is still in use");
    delete request;
    return true;
  }
  OPEL_LOG_DBG(CAM, "OpenCV Bin is no longer used");
  ...
  this->mRequestCount--;
  if (this->mRequestCount == 0) {
    OPEL_LOG_DBG(CAM, "No request in pipeline");
    gst_element_set_state(this->mPipeline, GST_STATE_READY);
  }
...
{
  OPEL_LOG_ERR(CAM, "deleteRequest Start");
  ...
  for (iter; iter != iter_end; iter++) {
    ...
    if (request == req) {
      ...
      this->mRequestCount--;
      OPEL_LOG_DBG(CAM, "Current number of requests: %d", this->mRequestCount);
      if (this->mRequestCount == 0) {
        OPEL_LOG_DBG(CAM, "No request in pipeline");
        gst_element_set_state(this->mPipeline, GST_STATE_READY);
      }
  ...
  OPEL_LOG_WARN(CAM, "Cannot find request");
  return false;
}
...
bool CameraDevice::deleteRequest(dbusRequest *msg_handle, RequestType type)
{
  ...
  if (type == kRecordingStop)
    ...
    OPEL_LOG_WARN(CAM, "Given request type is inappropriate (%d)", type);
    return false;
  }
  ...
  for (iter; iter != iter_end; iter++) {
    ...
    if (req->getRequestType() == target_type) {
      ...
      if (msg_handle->pid == tmp->pid && msg_handle->camera_id == tmp->camera_id &&
        ...
        OPEL_LOG_DBG(CAM, "PID:%u, Camera ID:%u, File Path:%s", tmp->pid, tmp->camera_id, tmp->file_path);
        this->mRequestList->erase(iter);
        delete req;
  ...
  for (iter; iter != iter_end; iter++) {
    ...
    if (req->getRequestType() == target_type) {
      ...
      if (msg_handle->pid == tmp->pid && msg_handle->camera_id == tmp->camera_id &&
        ...
        OPEL_LOG_DBG(CAM, "PID:%u, Camera ID:%u, IP Addr:%s", tmp->pid, tmp->camera_id, tmp->ip_address);
        this->mRequestList->erase(iter);
        delete req;
...
static gboolean busCB(GstBus *bus, GstMessage *message, gpointer user_data)
{
  ...
  switch (GST_MESSAGE_TYPE(message)) {
    case GST_MESSAGE_ERROR:
      ...
      OPEL_LOG_ERR(CAM, "Error received from element %s: %s",
          GST_OBJECT_NAME(message->src), err->message);
      OPEL_LOG_ERR(CAM, "Debugging information: %s",
    ...
    case GST_MESSAGE_EOS:
      OPEL_LOG_DBG(CAM, "End-Of-Stream reached");
      break;
  }
  return true;
}
...
{
  OPEL_LOG_DBG(CAM, "Time out callback invoked");
...
static GstPadProbeReturn padProbeCB(GstPad *pad, GstPadProbeInfo *info,
    gpointer _request)
{
  OPEL_LOG_DBG(CAM, "Pad probe callback invoked");
    ...
    gpointer _request)
{
  ...
  OPEL_LOG_DBG(CAM, "EOS reached to last pad");
...
static unsigned fileWrite(const char *file_path, char *buffer, unsigned size)
{
  ...
  if (!fp) {
    OPEL_LOG_ERR(CAM, "File open failed: %s", file_path);
    return ret;
  }

  ret = fwrite((char*)buffer, sizeof(char), size, fp);
  if (ret != size) {
    OPEL_LOG_ERR(CAM, "File write failed: %s", file_path);
    ret = 0;
  }
...
static GstFlowReturn jpegBufferFromSinkCB(GstElement *ele, gpointer _request)
{
  ...
  if (gst_buffer_map(buffer, &map, GST_MAP_READ)) {
    if (map.size != fileWrite(msg_handle->file_path,
      ...
      OPEL_LOG_ERR(CAM, "Fail to write");
      return GST_FLOW_OK;
    }
  }
...
static GstFlowReturn openCVBufferFromSinkCB(GstElement *ele, gpointer _request)
{
  ...
  GstBuffer *buffer;
  GstMapInfo map;
  OPELRawRequest *raw_request = (OPELRawRequest*)_request;
