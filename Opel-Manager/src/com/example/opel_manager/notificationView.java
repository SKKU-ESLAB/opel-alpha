...
public class notificationView extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.dynamic_noti_page);
		...
		while(jp.hasMoreValue()){
			...
			if(ret[0].equals("text")){
				...
				try {
					...
					if(isNoti.equals("2") ){
						is = new FileInputStream(new File(globalData.getInstance().getCloudStoragePath(), ret[1]));
					}
					else {
						is = new FileInputStream(new File(globalData.getInstance().getRUIStoragePath(), ret[1]));
		...
		switch (item.getItemId()) {
			case android.R.id.home:
				...
				if(checkNoti.equals("1")){
					Intent intent = new Intent(notificationView.this, eventLoggerView.class);
