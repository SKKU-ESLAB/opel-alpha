...
class OPEL_Socket
{
	...
	public:
		OPEL_Socket(int sock_fd, uint8_t type);
		~OPEL_Socket();
...
class OPEL_MSG
{
	...
	public:
		OPEL_MSG();
		OPEL_MSG(OPEL_Socket *op_sock);
		~OPEL_MSG();

		OPEL_Header *get_header();
		uint32_t get_req_id();
...
class queue_data_t
{
	public:
		...
		queue_data_t();
		queue_data_t(OPEL_Socket *op_sock);
		~queue_data_t();
		
		void call_handler(void);
};

class OPEL_Comm_Queue
{
	private:
...
class socket_set
{
	private:
		...
		OPEL_Socket* sockets[MAX_CLIENTS];
...
class cv_set
{
	...
	public:
		...
		int sch_to_sig(uint32_t reqid, OPEL_Comm_Queue *queue, queue_data_t *queue_data);
...
class OPEL_Server
{
	private:
		...
		//Thread safe
		OPEL_Comm_Queue read_queue;
		OPEL_Comm_Queue write_queue;
	...
	public:
		OPEL_Server(const char *intf_name);
		OPEL_Server(const char *intf_name, Comm_Handler serv_handler);
		~OPEL_Server();
...
class OPEL_Client
{
	...
	public:
		OPEL_Client(const char *intf_name, Comm_Handler onConnect);
		OPEL_Client(const char *intf_name, Comm_Handler client_handler, Comm_Handler onConnect);
		~OPEL_Client();

		void SetClientHandler(IN Comm_Handler cli_handler);
		int file_write(IN const char *filePath, IN OPEL_MSG *req_msg = NULL, IN Comm_Handler ack_file_handler = NULL);
		int msg_write(IN const char *buf, IN int len, IN OPEL_MSG *req_msg = NULL, IN Comm_Handler ack_msg_handler = NULL);
